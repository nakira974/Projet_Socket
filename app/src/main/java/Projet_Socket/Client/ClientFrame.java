...
import Projet_Socket.Utils.File.FileObject;
...
public class ClientFrame extends JFrame implements ActionListener, ISocketListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == connectButton) {
			...
			if (socket_client != null) {
				if (search.isEmpty()) {
					try {
						socket_client.writeSocket("VIEW_ALL_FILE");
					} catch (IOException ex) {
						ex.printStackTrace();
					}
				}
					...
					try {
						socket_client.writeSocket("SEARCH_FILE" + "--" + search);
					} catch (IOException ex) {
						ex.printStackTrace();
					}
				}
			...
			if (list.getSelectedIndex() != -1) {
				...
				try {
					socket_client.writeSocket("DOWNLOAD_FILE" + "--" + str);
				} catch (IOException ex) {
					ex.printStackTrace();
				}
			}
	...
	@Override
	public String chooserFileToSave(FileObject dataFile) {
		JFileChooser fileChooser = new JFileChooser();
		int returnVal = fileChooser.showOpenDialog(this);
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			File fileToSave = fileChooser.getSelectedFile();
